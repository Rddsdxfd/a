...
def get_crypto_prices():
    try:
        ...
        if response.status_code == 200:
            ...
            return data.get("bitcoin", {}).get("usd"), data.get("litecoin", {}).get("usd")
        else:
...
def send_message(chat_id, message):
    try:
        ...
        return bot.send_message(chat_id, message).message_id
    except Exception as e:
...
def main():
    ...
    if old_btc_price is None or old_ltc_price is None:
        return  # Exit if initial prices couldn't be obtained
    ...
    while True:
        ...
        if new_btc_price is None or new_ltc_price is None:
            continue  # Skip this iteration if current prices couldn't be obtained
        ...
        if has_price_changed(old_btc_price, new_btc_price):
            ...
            edit_message(601140054, message_id, f"BTC: ${new_btc_price}\nLTC: ${old_ltc_price}")
        ...
        if has_price_changed(old_ltc_price, new_ltc_price):
            ...
            edit_message(601140054, message_id, f"BTC: ${old_btc_price}\nLTC: ${new_ltc_price}")
